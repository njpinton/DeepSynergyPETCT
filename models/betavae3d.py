...
def build_dual_encoder(input_shape=(16, 32, 32, 1), inter_dim=16, latent_dim=32, filters=16, layers_n=3, merge_type='add'):
    ...
    for i in range(layers_n):
        x1 = layers.Conv3D(e1_filters, 3, activation="relu", strides=2, padding="same")(x1)
        e1_filters *= 2

    x2 = e2_input
    for i in range(layers_n):
        x2 = layers.Conv3D(e2_filters, 3, activation="relu", strides=2, padding="same")(x2)
        e2_filters *= 2
...
def build_dual_decoder(input_shape=(32, 32, 1), latent_dim=32, filters=16, layers_n=3):
    ...
    x1 = layers.Dense(s // div_ * n // div_ * n // div_ * div_ * filters, activation="relu")(latent_inputs)
    x1 = layers.Reshape((s // div_, n // div_, n // div_, div_ * filters))(x1)

    d1_filters = d1_filters * 2 ** (layers_n - 1)
    for i in range(layers_n):
        x1 = layers.Conv3DTranspose(d1_filters, 3, activation="relu", strides=2, padding="same")(x1)
        d1_filters //= 2

    # d1_outputs = layers.Conv2DTranspose(c, 3, activation="sigmoid", strides=2, padding="same")(x1)
    d1_outputs = layers.Conv3DTranspose(c, 3, activation="relu", padding="same")(x1)

    # build branch 2 of decoder
    x2 = layers.Dense(s // div_ * n // div_ * n // div_ * div_ * filters, activation="relu")(latent_inputs)
    x2 = layers.Reshape((s // div_, n // div_, n // div_, div_ * filters))(x2)

    d2_filters = d2_filters * 2 ** (layers_n - 1)
    for i in range(layers_n):
        x2 = layers.Conv3DTranspose(d2_filters, 3, activation="relu", strides=2, padding="same")(x2)
        d2_filters //= 2

    # d2_outputs = layers.Conv2DTranspose(c, 3, activation="sigmoid", strides=2, padding="same")(x2)
    d2_outputs = layers.Conv3DTranspose(c, 3, activation="relu", padding="same")(x2)
...
def build_dual_betavae(input_shape=(32, 32, 1), inter_dim=16, latent_dim=32, filters=16, layers_n=3, merge_type='add'):
...
if __name__ == "__main__":
    ...
    input_shape = [total_slices, image_size, image_size, 1]

    model = build_dual_betavae(input_shape, inter_dim, latent_dim, filters, layers_n)
